...
class AdminCmsUsersController extends \crocodicstudio\crudbooster\controllers\CBController
{
    ...
    public function cbInit()
    {
        ...
        if (CRUDBooster::myPrivilegeId() == 2) {
            $this->col[] = array(
                "label" => "Estado",
                "name" => "estado",
                "callback_php" => '$row->estado ? "Activo" : "Inactivo"'
            );
        ...
        if (CRUDBooster::myPrivilegeId() == 2) {
            $this->col[] = array(
                "label" => "Linkers Actuales",
                "name" => "afiliaciones_actuales"
            );
            $this->col[] = array(
                "label" => "Vistas Actuales",
                "name" => "vistas_actuales"
            );
            ...
            $this->col[] = array(
                "label" => "Linkers Solicitud",
                "name" => "(SELECT IFNULL( (select afiliados from solicitudes_de_pago where cms_users_id=cms_users.id order by solicitudes_de_pago.id desc limit 1),0)) as linkers_solicitados"
            );
            $this->col[] = array(
                "label" => "Vistas Solicitud",
                "name" => "(SELECT IFNULL( (select vistas from solicitudes_de_pago where cms_users_id=cms_users.id order by solicitudes_de_pago.id desc limit 1),0)) as vistas_solicitadas"
            );
            $this->col[] = array(
                "label" => "Monto Solicitado",
                "name" => "(SELECT IFNULL( (select monto from solicitudes_de_pago where cms_users_id=cms_users.id order by solicitudes_de_pago.id desc limit 1),0)) as monto"
            );
            $this->col[] = array(
                "label" => "Estado Solicitud",
                "name" => "(select nombre from estados where id= ( select estados_id from solicitudes_de_pago where cms_users_id=cms_users.id order by solicitudes_de_pago.id desc limit 1)) as estado_solicitud",
                "callback_php" => '$row->estado_solicitud ? ( $row->estado_solicitud == "solicitado" ? "<label class=\"label label-info\">$row->estado_solicitud</label>" : "<label class=\"label label-success\">$row->estado_solicitud</label>" ) : "Sin solicitar"'
            );
            $this->col[] = array(
                "label" => "Fecha Solicitud",
                "name" => "(select created_at from solicitudes_de_pago where cms_users_id=cms_users.id order by solicitudes_de_pago.id desc limit 1) as fecha_solicitud",
                "callback_php" => '$row->fecha_solicitud ? date("d/m/y",strtotime($row->fecha_solicitud)) : "Ninguna"'
            );
            $this->col[] = array(
                "label" => "Patrocinador",
                "name" => "cms_users_id",
            ...
            $this->col[] = [
                ...
                "name" => "created_at",
                "callback_php" => 'date("d/m/Y h:i A",strtotime($row->created_at))'
            ];
            $this->col[] = [
                "label" => "Fecha Activación",
                "name" => "activated_at",
                "callback_php" => '$row->activated_at ? date("d/m/Y h:i A",strtotime($row->activated_at)) : "sin información"'
            ];
            //$this->col[] = array("label"=>"Tipo","name"=>"id_cms_privileges","join"=>"cms_privileges,name");
        }
        if (CRUDBooster::myPrivilegeId() == 3) {
            $this->col[] = array(
                "label" => "N° Total de Linkers",
                "name" => "( SELECT COUNT(*) FROM cms_users b where b.cms_users_id=cms_users.id and b.estado=1) as linkers_totales"
            );
        }
        ...
        $this->form[] = array(
            ...
            "name" => "photo",
            "type" => "upload",
            "help" => "Resolución recomendada: 200x200px",
            'validation' => 'image|max:10000',
            'resize_width' => 90,
            'resize_height' => 90
        );
        $this->form[] = array(
        ...
        $this->form[] = array(
            ...
            "name" => "password",
            "type" => "password",
            "help" => "Dejar vacío si no se cambiará la contraseña"
        );
        $this->form[] = array(
            "label" => "Repita Nueva Contraseña",
            "name" => "password_confirmation",
            "type" => "password",
            "help" => "Dejar vacío si no se cambiará la contrasñea"
        );
        ...
        if (CRUDBooster::myPrivilegeId() == 2) {
            $this->addaction[] = [
                ...
                'icon' => 'fa fa-dollar',
                'color' => 'warning',
                'showIf' => '[estado_solicitud]=="solicitado"'
            ];
            $this->addaction[] = [
                'label' => '',
                'url' => '#activar_modal',
            ...
            $this->sub_module[] = [
                'label' => 'pagos',
                'path' => 'ganancias',
                'foreign_key' => 'cms_users_id',
                'button_color' => 'success',
                'button_icon' => 'fa fa-dollar',
                'parent_columns' => 'name',
                'parent_columns_alias' => 'Linker'
            ];
        ...
        $this->sub_module[] = [
            ...
            'foreign_key' => 'cms_users_id',
            'button_color' => 'info',
            'button_icon' => 'fa fa-group',
            'parent_columns' => 'name',
            'parent_columns_alias' => 'Patrocinador'
        ];
        $this->script_js = "
        ...
        if (CRUDBooster::myPrivilegeId() == 3) {
            ...
            // Si es un usuario normal, le aparecerán todas estas estadísticas listadas en este if
            ...
            $this->index_statistic[] = [
                'label' => 'N° Actual de Vistas',
                'count' => $vistas_actuales,
                'icon' => 'fa fa-video-camera',
                'color' => 'blue',
                'width' => 'col-sm-2'
            ];
            $this->index_statistic[] = [
                'label' => 'N° Actual de Linkers afiliados',
                'count' => $hijos_actuales,
                'icon' => 'fa fa-users',
                'color' => 'blue',
                'width' => 'col-sm-2'
            ];
            $this->index_statistic[] = [
                ...
                'count' => DB::table($this->table)->where('cms_users_id', $id)->where('estado', 1)->count(),
                'icon' => 'fa fa-user-times',
                'color' => 'blue',
                'width' => 'col-sm-2'
            ];
            $this->index_statistic[] = [
                'label' => 'N° de Linkers sin Activarse',
                'count' => DB::table($this->table)->where('cms_users_id', $id)->whereNull('estado')->count(),
                'icon' => 'fa fa-user-times',
                'color' => 'blue',
                'width' => 'col-sm-2'
            ];
            $this->index_statistic[] = [
                'label' => 'Vistas por Cobrar',
                'count' => $vistas_x_cobrar,
                'icon' => 'fa fa-download',
                'color' => 'blue',
                'width' => 'col-sm-2'
            ];
        ...
        if (CRUDBooster::myPrivilegeId() == 2) {
            ...
            if ($request['parent_table'] == 'cms_users') {
                ...
                $this->index_statistic[] = [
                    ...
                    'count' => ' $' . $monto_ultima_solicitud,
                    'icon' => 'fa fa-trophy',
                    'color' => 'blue',
                    'width' => 'col-sm-3 col-lg-3'
                ];
                $monto_solicitud_premium = $this->compensaciones->getMontoSolicitudPremium($user->id);
                //if($user->premium){
                $this->index_statistic[] = [
                    'label' => 'Monto por Linkers de Linkers -> Última Solicitud: Monto ganado en la última solicitud generado por linkers de linkers',
                    'count' => ' $' . $monto_solicitud_premium,
                    'icon' => 'fa fa-trophy',
                    'color' => 'blue',
                    'width' => 'col-sm-3 col-lg-3'
                ];
    ...
    public function hook_before_edit(
        &$postdata,
        $id
    )
    {
    ...
    public function changeUserEstado($id)
    {
        ...
        if (!$user->estado) {
            ...
            if ($user->cms_users_id) {
                $idpadre = $user->cms_users_id;
                //aumentamos la cantidad de afiliados actuales del padre en 1
                DB::table('cms_users')
                    ->where('id', $idpadre)
                    ->increment('afiliaciones_actuales', 1);             
                $abuelo = $this->getPadre($idpadre);
                if (!empty($abuelo)) {
                    //aumentamos la cantidad de afiliados actuales del abuelo en 1
                    DB::table('cms_users')
                        ->where('id', $abuelo->id)
                        ->increment('nietos_actuales', 1);
    ...
    public function getPadre($id)
    {
        $user = DB::table('cms_users')->where('id', $id)->first();
        if ($user->cms_users_id) {
            $padre = DB::table('cms_users')->where('id', $user->cms_users_id)->first();
            return $padre;
        } else {
            return null;
        }
