	...
	public function cbInit() {
		...
		if ( CRUDBooster::myPrivilegeId() == 2 ) {
			$this->col[] = array(
				"label"        => "Premium",
				"name"         => "premium",
				"callback_php" => '$row->premium ? "Sí" : "No"'
			);
		...
		$this->form[]   = array(
			...
			"name"       => "email_paypal",
			'type'       => 'email',
			'validation' => 'email|unique:cms_users,email_paypal,' . CRUDBooster::getCurrentId()
		);
		$this->form[]   = array(
		...
		if ( CRUDBooster::myPrivilegeId() == 2 ) {
			...
			$this->sub_module[] = [
				...
				'parent_columns'       => 'name,premium',
				'parent_columns_alias' => 'Patrocinador,Premium'
			];
			$this->sub_module[] = [
				'label'                => 'pagos',
				'path'                 => 'ganancias',
				...
				'parent_columns'       => 'name,premium',
				'parent_columns_alias' => 'Linker,Premium'
			];
		...
		if ( CRUDBooster::myPrivilegeId() == 2 ) {
			...
			if ( $request['parent_table'] == 'cms_users' ) {
				...
				if ( $user->premium ) {
					...
					$this->index_statistic[] = [
						'label' => 'Ganancia Premium Actual: Ganancia generada por los linkers de sus linkers actualmente',
						'count' => ' $' . $ganancia_premium,
						'icon'  => 'fa fa-usd',
						'color' => 'blue',
						'width' => 'col-sm-2 col-lg-3'
					];
				}
				...
				if ( $user->premium ) {
					$this->index_statistic[] = [
						'label' => 'Monto Premium Última Solicitud: Monto de la última solicitud ganado por linkers de linkers',
