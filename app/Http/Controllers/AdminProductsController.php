...
class AdminProductsController extends \crocodicstudio\crudbooster\controllers\CBController {
	...
	public function cbInit() {
		...
		$this->col[] = [ "label" => "Tipo", "name" => "product_type_id", "join" => "product_types,name" ];
		$this->col[] = [ "label" => "Empresa", "name" => "enterprise_id", "join" => "empresas,nombre" ];
	...
	public function getIndex() {
			...
			foreach ( $products as &$product ) {
				if ( $product->product_type_id == 1 ) {
					$product->ogp = ProductOPGService::getProductInformation( $product->value );
				}
			}
